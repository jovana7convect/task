...
export abstract class BaseNgrxService {
  ...
  public selectApiVersion(): Observable<string> {
    return this.selectOnce((s: AppState) => s.apiVersion || 'v1');
  }
  ...
  public withApiVersion<T>(
    project: (apiVersion: string) => ObservableInput<T>,
  ): Observable<T> {
    return this.selectApiVersionOnce().pipe(switchMap(project));
  }

  public dispatchNow(action: Action): void {
