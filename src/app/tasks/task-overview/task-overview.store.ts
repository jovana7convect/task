...
import { setAppVersion } from '../../store/app.actions';
...
export class TasksOverviewStore extends BaseComponentStore<TaskStoreState> {
  ...
  public readonly tasks$: Observable<Task[]> = this.select((s) => s.tasks).pipe(
    map(this.tasksSelectors.selectAll),
  );

  constructor(private readonly service: TaskOverviewService) {
    super(service);
  ...
  public refresh: () => void = this.effect(($) =>
    $.pipe(
  ...
  public setAppVersion(version: string) {
    console.log(version);
    this.service.dispatchNow(setAppVersion(version));
  }
}
