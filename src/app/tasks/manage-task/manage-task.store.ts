...
export class ManageTaskStore extends BaseComponentStore<ManageTaskStoreState> {
  ...
  public readonly taskId$: Observable<string | null> = this.select(
    (s) => s.task?.uuid || null,
  );
  public readonly taskIsNew$: Observable<boolean> = this.taskId$.pipe(
    map((id) => !id),
  );
  ...
  public saveTask(task: Task): Observable<Task> {
    return this.apiVersion$.pipe(
      ...
      switchMap((apiVersion) => this.service.saveTask(apiVersion, task)),
